...
export default function ExecutionBrief({
  ...
  return (
    <div className="mt-4">
      ...
      {loading && (
        <div className="fade-in-up flex items-center gap-3 py-3 px-4 rounded-lg bg-white/5 border border-white/10">
          <div className="w-4 h-4 border-2 border-white/20 border-t-white/70 rounded-full animate-spin shrink-0" />
          <span className="text-sm text-white/50">Analyzing with Qwen2.5…</span>
        </div>
      )}

      {error && !loading && (
        <div className="fade-in-up py-2.5 px-4 rounded-lg bg-white/5 border border-white/10 text-sm text-white/40 italic">
          {error}
        </div>
      )}

      {brief && (
        <div className="fade-in-up rounded-xl border border-white/10 bg-[#111113] overflow-hidden">
          {/* Header */}
              ...
              <ul className="space-y-1 mt-1">
                {brief.key_risks.map((risk, i) => (
                  <li
                    ...
                    className="flex items-start gap-2 text-sm text-white/80"
                  >
                    <span className="shrink-0 text-amber-400/70 mt-0.5">▲</span>
                    {risk}
                  </li>
                ))}
              </ul>
